...
PANEL_CONFIG = [
    # Page 1  (indices 0-8)
    ("Brokerages",               "Sheet 1"),
    ("PSU Banks",                "Sheet2"),       # no space — exact xlsx name
    ...
    # Page 2  (indices 9-17)
    ("Pharmaceuticals",          " Sheet 10"),    # leading space in xlsx
...
# ---------------------------------------------------------------------------
# LOAD SHEET DATA (generated by extract_sheets.py)
# ---------------------------------------------------------------------------
with open("sheet_data.json", "r") as f:
    raw_sheet_data = json.load(f)
...
def _save_sheet_data() -> None:
    with open("sheet_data.json", "w") as fh:
        json.dump(raw_sheet_data, fh, indent=2)
...
# into sheet_data.json so it is the permanent single source of truth.
...
for _panel in panels:
    ...
    if _pid not in raw_sheet_data:
        continue  # user-created panel — not in sheet_data.json
