...
├── extract_sheets.py         ← One-time helper: reads blp work.xlsx → sheet_data.json
├── extract_all.py            ← Bulk extract helper
│
├── sheet_data.json           ← Symbol list (generated by extract_sheets.py)
...
| `sheet_data.json` | Base symbol list from Excel | ✅ Re-generate with `extract_sheets.py` |
...
| Server won't start | Ensure `sheet_data.json` exists; run `python extract_sheets.py` if missing |
